...
def _load_matlab_nbs_results(mat_path: str, alpha: float) -> dict:
    ...
    if pval is not None and np.size(pval) > 0:
        comp_pvals = np.atleast_1d(np.asarray(pval, dtype=float)).tolist()

    comp_masks: list[np.ndarray] = []
    for comp in _cell_to_list(con_mat):
...
parser.add_argument('--nuisance', type=str, default=None,
                    help="Comma-separated list of covariate names to treat as nuisances (default: all except regressor)")
...
default_nuis = nuisance_cli if nuisance_cli else ["sex", "age"]
for cov in default_nuis:
...
base_dir      = join(dutils.ANARESULTSPATH, "controls_vs_patients", "nbs",group,modality)
...
nuisance_terms_preview = nuisance_cli if nuisance_cli else [k for k in designmat_dict.keys() if k != regressor_name]
...
if nuisance_cli:
    ...
    nuisance_terms = [k for k in designmat_dict.keys() if k != regressor_for_bct]
...
param_tag  = (
    f"group-{_slugify_fragment(group)}_parc-{_slugify_fragment(parc_scheme)}"
    f"_scale-{scale}_diag-{_slugify_fragment(diag)}_perm-{_slugify_fragment(args.permtest)}"
    f"_nperm-{args.nperm}_th-{args.t_thresh}_reg-{_slugify_fragment(regressor_name)}"
    f"_nuis-{nuis_tag}_lobes-{_slugify_fragment(lobes_choice)}_test-{test_type_tag}_tail-{tail_tag}{selection_tag}"
)
...
if export_paths:
    ...
    if run_matlab:
        ...
        for suffix, output_mat in matlab_outputs:
            ...
            res = _load_matlab_nbs_results(output_path, alpha=args.alpha)
