...
class ColorBar():
    ...
    def load_simple_cmap(self, scheme="rgb", plotly=False, name="simple_cmap"):
        """
        Return a simple colormap (Matplotlib or Plotly) without loading FSL maps.

        Parameters
        - scheme: str, one of the presets below. Examples:
            'rgb'              -> red → green → blue
            'redwhiteblue'     -> red → white → blue (diverging)
            'bluewhiteorange'  -> blue → white → orange (diverging)
            'greenwhitepurple' -> green → white → purple (diverging)
            'grayscale'        -> black → white (sequential)
            'spectrum_light'   -> blue → cyan → yellow → red (sequential)
        - plotly: bool, if True returns a Plotly colorscale, else a Matplotlib colormap
        - name: str, name for the Matplotlib colormap

        Returns
        - Matplotlib LinearSegmentedColormap or Plotly colorscale list
        """
        scheme = (scheme or "").lower()

        # Define simple palettes using color names. Stops are evenly spaced.
        presets = {
            "rgb": ["red", "green", "blue"],
            "redwhiteblue": ["red", "white", "blue"],
            "rwb": ["red", "white", "blue"],
            "bluewhiteorange": ["dodgerblue", "white", "orangered"],
            "bwo": ["dodgerblue", "white", "orangered"],
            "greenwhitepurple": ["mediumseagreen", "white", "purple"],
            "grayscale": ["black", "white"],
            # A light spectrum without relying on external files
            "spectrum_light": ["blue", "cyan", "yellow", "red"],
        }

        # Single-color convenience and fallback
        simple_colors = {"red", "blue", "green", "orange", "purple", "black", "white", "gray", "grey", "cyan", "magenta", "yellow"}
        colors = None
        if scheme.startswith("solid:") or scheme.startswith("solid-"):
            color = scheme.split(":", 1)[1] if ":" in scheme else scheme.split("-", 1)[1]
            colors = [color, color]
        elif scheme in simple_colors:
            colors = [scheme, scheme]
        else:
            if scheme not in presets:
                scheme = "redwhiteblue"
            colors = presets[scheme]
        # Create colormap from list of colors (evenly spaced)
        cmap = LinearSegmentedColormap.from_list(name, colors)
        if plotly:
            return self.__matplotlib_to_plotly(cmap)
        return cmap
