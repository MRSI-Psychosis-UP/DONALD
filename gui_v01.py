...
try:
    ...
    from PyQt6.QtWidgets import QApplication, QMainWindow, QSlider, QWidget, QVBoxLayout, QHBoxLayout
    from PyQt6.QtWidgets import QFileDialog, QPushButton, QStyle, QSpinBox, QSplitter
    from PyQt6.QtWidgets import QLabel
    from PyQt6.QtGui import QFont
    from PyQt6.QtWidgets import QTableWidget, QTableWidgetItem
    from PyQt6.QtCore import Qt
    QT_LIB = 6
    # Provide PyQt5-style enum aliases for the existing code path
...
def _load_tensor_from_path(path: Path):
    img = nib.load(str(path))
    data = img.get_fdata()
    if data.ndim > 3:
        data = data[..., 0]
    data = np.asarray(data)
    data = np.flip(data, axis=2) if data.ndim >= 3 else data
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        base_datapath = getattr(utils, "DATAPATH", getattr(utils, "DEVDATAPATH", "."))
        self.DATAPATH = os.path.join(base_datapath,"MRSI_reconstructed")
        ...
        self.tensor3D_current = None
        self.header = None
        self.unique_ids = {}
        self.session_path_override = os.getenv("MRSI_ENTRY_PATH")
        if self.session_path_override and Path(self.session_path_override).exists():
            data, header = _load_tensor_from_path(Path(self.session_path_override))
            self.tensor3D_current = data
            self.header = header
            self.unique_ids = {
                Path(self.session_path_override).name: {
                    ft: True for ft in ["Qmask", "Conc", "Basic", "Holes", "BasicEigenA", "BasicEigenB", "BasicEigenC"]
                }
            }
        else:
            try:
                ...
                print(f"Failed to load default dataset: {exc}")

        if self.tensor3D_current is None:
            self.tensor3D_current = np.zeros((1, 1, 1))
            self.header = None
    ...
    def table_select(self, row, column):
        if getattr(self, "session_path_override", None):
            return
