...
class  MRIData:
    ...
    def get_connectivity_path(self,mode,parc_scheme,scale,npert=50,filtoption=""):
        ...
        atlas   = f"chimera{parc_scheme}_scale{scale}" if "cubic" not in parc_scheme else f"cubic{scale}mm"

        if mode=="mrsi":
            filename = f"{self.prefix}_atlas-{atlas}_npert-{npert}_filt-{filtoption}_desc-connectivity_mrsi.npz"
        elif mode == "dwi": 
            filename = f"{self.prefix}_atlas-{atlas}_desc-connectivity_dwi.npz"
